...
class CostTracker: ObservableObject {
    ...
    init() {
        ...
        // Detect local Clawdbot via centralized path manager
        hasClawdbot = ClawdbotPathManager.shared.isDetected
    }
    ...
    func refreshAll() async {
        ...
        do {
            ...
            if isRemote {
                ...
                oauthUsage = try? await remote.fetchOAuthUsage()
            } else {
    ...
    private func loadClawdbotConfig() -> [String: Any] {
        guard let basePath = ClawdbotPathManager.shared.basePath else { return [:] }
        let configPath = basePath + "/clawdbot.json"
        guard let data = try? Data(contentsOf: URL(fileURLWithPath: configPath)),
              let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any] else { return [:] }
        return json
    }

    private func getAgentContextLimit(config: [String: Any], agentId: String) -> Int {
        let agents = config["agents"] as? [String: Any] ?? [:]
        let defaults = agents["defaults"] as? [String: Any] ?? [:]
        let defaultLimit = defaults["contextTokens"] as? Int ?? 200_000

        if let list = agents["list"] as? [[String: Any]] {
            for agent in list {
                if agent["id"] as? String == agentId {
                    return agent["contextTokens"] as? Int ?? defaultLimit
                }
            }
        }
        return defaultLimit
    ...
    private func getAgentConfiguredModel(config: [String: Any], agentId: String) -> String {
        let agents = config["agents"] as? [String: Any] ?? [:]
        let defaults = agents["defaults"] as? [String: Any] ?? [:]
        let defaultModel = (defaults["model"] as? [String: Any])?["primary"] as? String ?? "unknown"

        if let list = agents["list"] as? [[String: Any]] {
            for agent in list {
                if agent["id"] as? String == agentId {
                    if let model = (agent["model"] as? [String: Any])?["primary"] as? String {
                        return model
                    }
                    return defaultModel
                }
    ...
    private func getAgentAuthMode(agentId: String) -> String {
        guard let agentsPath = ClawdbotPathManager.shared.agentsPath else { return "unknown" }
        let authPath = agentsPath + "/" + agentId + "/agent/auth-profiles.json"
        guard let data = try? Data(contentsOf: URL(fileURLWithPath: authPath)),
              let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
              let lastGood = (json["lastGood"] as? [String: Any])?["anthropic"] as? String,
              let profiles = json["profiles"] as? [String: Any],
              let profile = profiles[lastGood] as? [String: Any] else { return "unknown" }
        return profile["type"] as? String ?? "unknown"
    }
    ...
    private func parseActiveSessions() -> [SessionInfo] {
        ...
        for agentDir in agentDirs {
            ...
            let displayName = agentDir.replacingOccurrences(of: "-", with: " ").prefix(1).uppercased() + agentDir.replacingOccurrences(of: "-", with: " ").dropFirst()
            let contextLimit = getAgentContextLimit(config: config, agentId: agentDir)
            let configuredModel = getAgentConfiguredModel(config: config, agentId: agentDir)
            let authMode = getAgentAuthMode(agentId: agentDir)
            ...
            for sessionFile in sessionFiles {
                ...
                for line in lines.reversed() {
                    if line.contains("\"compaction\"") || line.contains("\"safeguard\"") {
                        compactions += 1
                        continue
                    }
                    ...
                    if line.contains("\"usage\"") && line.contains("\"cost\"") {
                        ...
                        if lastContextTokens == 0 {
                            ...
                            lastContextTokens = input + cacheRead + cacheWrite + output
